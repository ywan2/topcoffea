...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        mu['FO']      = isFOMuon(mu.pt, mu.conePt , mu.jetBTagDeepFlav, mu.mvaTTH, mu.jetRelIso, year)
        mu['isFO']    = mu['isPres'] & mu['FO']
        mu['isTight'] = tightSelMuon(mu.isFO, mu.mvaTTH)
        presel_mu=mu[mu.isPres] # for electron cleaning
        ...
        e['FO']      = isFOElec( e.conePt, e.jetBTagDeepFlav, ttH_idEmu_cuts_E3(e.hoe, e.eta, e.deltaEtaSC, e.eInvMinusPInv, e.sieie), e.convVeto, e.lostHits, e.mvaTTH, e.jetRelIso, e.mvaFall17V2noIso_WP80, year)

        e['isClean'] = isClean(e, presel_mu, drmin=0.05)
        e['isFO']  = e['isPres'] & e['FO'] & e['isClean']
        e['isTight'] = tightSelElec(e.isFO, e.mvaTTH)

        FO_e = e['isFO']
        
        FOs=ak.concatenate([FO_m,FO_e],axis=1)

        lep1=FOs[ak.argmax(FOs.conePt,axis=-1,keepdims=True)]
