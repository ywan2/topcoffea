...
class AnalysisProcessor(processor.ProcessorABC):
    def __init__(self, samples, wc_names_lst=[], do_errors=False, do_systematics=False, dtype=np.float32):
        ...
        'ht'      : HistEFT("Events", wc_names_lst, hist.Cat("sample", "sample"), hist.Cat("channel", "channel"), hist.Cat("cut", "cut"), hist.Cat("sumcharge", "sumcharge"), hist.Cat("systematic", "Systematic Uncertainty"), hist.Bin("ht",     "H_{T} (GeV)", 50, 0, 1000)),
        })
    ...
    def process(self, events):
        ...
        levels = ['base', '1+bm2+bl2j', '1+bm2+bl4j', '1bm2j', '2+bm2j']
        selections.add('base',       (nElec+nMuon>=2))
        selections.add('1+bm2+bl2j', (nElec+nMuon>=2)&((nbtagsm>=1)&(nbtagsl>=2))&(njets>=2))
